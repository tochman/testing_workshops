...
describe('Purchase', () => {
  ...
  // it.only('is expected to emit "InitialDeployment" event', async () => {
  //   let contract
  //   expect(contract = await waffle.deployContract(serviceProvider, Purchase, [buyer.address]))
  //   .to.emit(contract, 'InitialDeploy')
  //   .withArgs(serviceProvider.address, buyer.address, 'An invite to purchase services has been created');
  // });
  ...
  describe('Buyer can initiate the purchase process', () => {
    ...
    it('is expected to emit "Initialize" event', async () => {
      ...
      .withArgs(buyer.address, serviceProvider.address, 'Software Development', 10, 'h');
    });
